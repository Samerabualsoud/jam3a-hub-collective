...
-- Create a function to get all profiles for admin users with proper return type
...
COMMENT ON FUNCTION public.get_profiles_for_admin() IS 'Gets all profiles if user is admin, otherwise returns only the current user profile';
